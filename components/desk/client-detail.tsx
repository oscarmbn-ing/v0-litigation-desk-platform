...
export function ClientDetail({
  ...
  const getJudicialDataForCase = (caseId: string) => {
    ...
    return {
      ...base,
      currentStage: 2,
      stageDates: { 1: "15/10/2025", 2: "20/10/2025" },
      ...
      tasks: [
        {
          id: 999,
          title: "Preparar Excepciones",
          due: "Mañana",
          status: "urgent",
          assignee: "José Herrera",
        }
      ],
      notebooks: {
        ...base.notebooks,
            ...
            2: [
              {
                ...
                type: "presentacion",
                icon: "doc",
                title: "Estudio de excepciones",
                desc: "Abogado analiza excepciones posibles.",
  ...
  const getTimelineRows = (caseId: string) => {
    ...
    return [...principalRows, ...apremioRows, ...incidenteRows]
      .sort((a, b) => parseDate(b.event.date) - parseDate(a.event.date))
      .map(({ event, notebook }) => {
        const tipo =
  ...
  const summaryMovements = useMemo(() => {
    ...
    const judicialSource = `${activeCaseId} | ${activeCase?.creditor ?? client.activeCreditor}`
    ...
    const protectionRows = client.protectionData.contracts.flatMap((contract) =>
      Object.entries(contract.history).flatMap(([stage, events], groupIdx) =>
        events.map((event, idx) => ({
          ...
          desc: event.desc,
          date: event.date,
          source: `${contract.type} | ${contract.detail}`,
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <CardContent className="p-8 md:p-10">
            <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
                ...
                <div className="flex items-center gap-2.5 relative">
                  {contextView === "judicial" && (
                    <>
                      ...
                      <div className="relative">
                        ...
                        >
                          {!activeCaseId.includes("Sin juicios") ? (
                            <>
                  ...
                  {contextView === "protection" && currentData && (
                    <>
                      ...
                      <span className="text-base text-slate-500">Contrato:</span>
                      <div className="relative">
                        <button
                          type="button"
                          className={`flex items-center gap-1.5 px-2 py-0.5 rounded-md border transition-all text-base ${client.protectionData.contracts.length > 1
                            ? "cursor-pointer hover:bg-slate-50 border-slate-200 bg-transparent"
                            : "border-transparent text-slate-500"
                            }`}
                          onClick={() =>
                            client.protectionData.contracts.length > 1 &&
                        ...
                        >
                          ...
                          {client.protectionData.contracts.length > 1 && (
                            <ChevronDown
                              size={14}
                              className={`text-slate-400 transition-transform ${showContractSelector ? "rotate-180" : ""
                                }`}
                            />
                          )}
                        </button>
                          ...
                          <div className="absolute top-full left-0 mt-1 w-72 bg-white rounded-xl shadow-xl border border-slate-100 py-2 z-50">
                            ...
                            {client.protectionData.contracts.map((contract) => (
                              <button
                                ...
                                className={`w-full text-left px-4 py-3 text-sm flex items-center gap-3 hover:bg-emerald-50 transition-colors ${contract.id === activeContractId
                                  ? "bg-emerald-50"
                                  : ""
                                  }`}
                              >
                                ...
                                <div>
                                  <p
                                    className={`font-bold ${contract.id === activeContractId
                                      ? "text-emerald-700"
                                      : "text-slate-700"
                                      }`}
                                  >
              ...
              <div className="flex bg-slate-100 p-1 rounded-lg">
                ...
                <button
                  type="button"
                  disabled={!client.protectionData.isActive}
                  onClick={() => client.protectionData.isActive && handleContextChange("protection")}
                  className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-semibold transition-all ${contextView === "protection"
                    ...
                    : "text-slate-500 hover:text-slate-700 hover:bg-slate-200"
                    } ${!client.protectionData.isActive ? "opacity-50 cursor-not-allowed filter grayscale" : ""}`}
                >
                  P. Patrimonial
                </button>
              </div>
              ...
              {contextView === "judicial" ? (
                <div className="flex flex-col gap-2 text-sm text-slate-600">
                  <div className="flex items-center justify-between">
                    <span><span className="font-bold text-slate-900">Tribunal:</span> {client.tribunal}</span>
                    <span><span className="font-bold text-slate-900">Tipo reclamación:</span> {client.tipoReclamacion}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span><span className="font-bold text-slate-900">Acreedor:</span> {activeCaseId.includes("Sin juicios") ? "No aplica" : (activeCase?.creditor ?? client.activeCreditor)}</span>
                    <span className="flex items-center gap-2">
                      <span className="font-bold text-slate-900">Índice de riesgo:</span>
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-semibold ${client.health === "critical"
      ...
      <Dialog open={showBitacoraModal} onOpenChange={setShowBitacoraModal}>
        <DialogContent className="max-w-[1100px] w-[95vw] p-0 gap-0 border-slate-200 rounded-2xl overflow-hidden shadow-xl">
                                    ...
                                    <table className="w-full table-fixed text-xs md:text-sm">
                                      <thead className="bg-slate-100">
                                        <tr className="text-left text-slate-500 border-b border-slate-200">
                                          <th className="px-2 py-2 font-semibold w-[120px]">Fecha</th>
                                          <th className="px-2 py-2 font-semibold">Tarea / movimiento</th>
                                          <th className="px-2 py-2 font-semibold w-[130px]">Cuaderno</th>
                                          <th className="px-2 py-2 font-semibold w-[150px]">Tipo</th>
                                          <th className="px-2 py-2 font-semibold w-[160px]">Responsable</th>
                                          <th className="px-2 py-2 font-semibold w-[80px] text-center">Estado</th>
                                          <th className="px-2 py-2 font-semibold w-[60px] text-center">Doc</th>
                                        </tr>
                                      </thead>
                                      <tbody>
                    ...
                    <div className="max-w-[600px] mx-auto px-6 md:px-10 py-10 space-y-6">
                      {selectedSummaryMovements.length > 0 ? (
                        <>
                          ...
                          {/* WhatsApp CTA Styled like the reference */}
                          <div className="mt-16 pt-10 border-t border-[#f2f4ff] text-center">
                            <p
                              className="text-[#475467] mb-5"
                              style={{
                                fontSize: '16px',
                                fontWeight: 400,
                                fontFamily: 'system-ui, -apple-system, sans-serif'
                            ...
                            >
                              ¿Tienes dudas sobre los avances? Revisémoslo juntos.
                            </p>
                            <a
                              href={`https://wa.me/56912345678`}
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex items-center gap-2.5 px-8 py-3 bg-white border border-[#e0e4f5] text-[#271d8e] rounded-full font-bold text-sm no-underline shadow-sm hover:border-[#271d8e] transition-all"
